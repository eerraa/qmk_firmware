...
#        error "Tomak79S split USB-C wiring must not use full-duplex serial."
#    endif
#    if SERIAL_USART_TX_PIN != GP1
#        error "Tomak79S split serial must use GP1/RX/D+ as the single-wire half-duplex line."
#    endif

static void tomak79s_disable_unused_split_tx_pin(void) {
    palSetLineMode(TOMAK_SPLIT_UNUSED_TX_PIN, PAL_MODE_INPUT_ANALOG);
}

void keyboard_pre_init_kb(void) {
    tomak79s_disable_unused_split_tx_pin();
    keyboard_pre_init_user();
}
#endif

#if defined(TOMAK_SPLIT_DEFER_SLAVE_RGB_UNTIL_SYNC) && defined(RGB_MATRIX_ENABLE)
static void tomak79s_defer_slave_rgb_until_sync(void) {
...
#endif
#if defined(TOMAK_SPLIT_DEFER_SLAVE_RGB_UNTIL_SYNC) && defined(RGB_MATRIX_ENABLE)
    tomak79s_defer_slave_rgb_until_sync();
#endif
}
